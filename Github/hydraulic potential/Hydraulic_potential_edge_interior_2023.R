...
### IMPORT DATA
hyd_pot_23 <- read.csv(file.path(path, "Potencial_Borda_Interior_Novo_20230727_2.csv"))

hyd_pot_test <- read.csv(file.path(path, "Potencial_Borda_Interior_Novo_20230727_Original.csv"))
...
ggplot(hyd_pot_23, aes(x = Data, y = mean_pot, group=Especie_Nome_Comum, color=Especie_Nome_Comum)) +
  geom_line(lwd=0.8) +
...
hyd_pot_23 %>%
    ggplot(aes(x = Data, y = mean_pot, group=Periodo, color=Periodo)) +
   stat_summary(fun.data = "mean_cl_boot", alpha = .8,
...
hyd_pot_23 %>%
  ggplot(aes(x = Data, y = mean_pot, group=Local, color=Local)) +
